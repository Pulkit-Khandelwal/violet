...
def create_hdf5_dataset(args):
    ...
    for current_subject in subjects_list:
        print(os.path.join(subjects_path, str(current_subject)+CT_name))
        print(os.path.join(subjects_path, str(current_subject)+MR_name))

        CT_data = nib.load(os.path.join(subjects_path, str(current_subject)+CT_name))
        CT_data = np.asanyarray(CT_data.dataobj)
        CT_data = np.nan_to_num(CT_data)
        MR_data = nib.load(os.path.join(subjects_path, str(current_subject)+MR_name))
        ...
        if args.is_test_set == 'True':
            print("this IS the test set")
        else:
            print("this is NOT the test set")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_test_set', type=str)
