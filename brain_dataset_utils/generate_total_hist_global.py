...
def create_hdf5_dataset(args):
    ...
    subjects_path=args.dataset_path
    CT_name=args.CT_name
    MR_name=args.MR_name
    subjects_list=args.subjects_list 
    print(f"data size: {len(subjects_list)}", subjects_list)

    hist_dataset = {}
    for idx, current_subject in enumerate(subjects_list):
        start = time.time()
        print(idx, current_subject)
        print(os.path.join(os.path.join(subjects_path, str(current_subject)+MR_name)))
        print(os.path.join(os.path.join(subjects_path, str(current_subject)+CT_name))) 

        sub_name = current_subject
        if modal == 'MR':
            MR_data = nib.load(os.path.join(os.path.join(subjects_path, str(current_subject)+MR_name)))
        else:
            MR_data = nib.load(os.path.join(subjects_path, str(current_subject)+CT_name))
        ...
        fig_out_path = subjects_path+f'/{modal}_hists_global_{args.height}/{sub_name}.png'
        os.makedirs(os.path.dirname(fig_out_path), exist_ok=True)
        if visualize:
            visualize_histogram(normalized_histograms, num_bins, fig_out_path)

        cum_hist = np.cumsum(normalized_histograms)
        fig_out_path = subjects_path+f'/{modal}_hists_global_{args.height}/{sub_name}_cum.png'
        if visualize:
        ...
        hist_diff *= scale
        fig_out_path = subjects_path+f'/{modal}_hists_global_{args.height}/{sub_name}_diff.png'
        if visualize:
...
def create_hdf5_dataset_avg(args):
    ...
    for idx, current_subject in enumerate(subjects_list):
        ...
        if modal == 'MR':
            MR_data = nib.load(os.path.join(current_subject, MR_name))
        else:
            MR_data = nib.load(os.path.join(current_subject, CT_name))
        MR_data = np.asanyarray(MR_data.dataobj)
        MR_data = np.nan_to_num(MR_data)
        ...
        normalized_histograms = histograms / histograms.sum(keepdims=True)
        normalized_histograms *= scale
        fig_out_path = subjects_path+f'/{modal}_hists_global_ver3/{sub_name}.png'
        if visualize:
            visualize_histogram(normalized_histograms, num_bins, fig_out_path)
        normalized_list.append(normalized_histograms)
        
        cum_hist = np.cumsum(normalized_histograms)
        fig_out_path = subjects_path+f'/{modal}_hists_global_ver3/{sub_name}_cum.png'
        if visualize:
            visualize_histogram(cum_hist, num_bins, fig_out_path)
        ...
        hist_diff = np.insert(hist_diff, 0, hist_diff[0])
        hist_diff *= scale
        fig_out_path = subjects_path+f'/{modal}_hists_global_ver3/{sub_name}_diff.png'
        if visualize:
            visualize_histogram(hist_diff, num_bins, fig_out_path)
...
def create_hdf5_dataset_colin(args):
    ...
    colin_path = subjects_path+'/colin27_t1_tal_lin_preprocessed.nii'
    ...
    normalized_histograms = histograms / histograms.sum(keepdims=True)
    normalized_histograms *= scale
    fig_out_path = subjects_path+f'/colin_hists_global_ver3/hist.png'
    os.makedirs(os.path.dirname(fig_out_path), exist_ok=True)
    if visualize:
        visualize_histogram(normalized_histograms, num_bins, fig_out_path)
    
    cum_hist = np.cumsum(normalized_histograms)
    fig_out_path = subjects_path+f'/colin_hists_global_ver3/hist_cum.png'
    if visualize:
        visualize_histogram(cum_hist, num_bins, fig_out_path)
    ...
    hist_diff = np.insert(hist_diff, 0, hist_diff[0])
    hist_diff *= scale
    fig_out_path = subjects_path+f'/colin_hists_global_ver3/hist_diff.png'
    if visualize:
        visualize_histogram(hist_diff, num_bins, fig_out_path)
    combined_histogram = np.stack((normalized_histograms, cum_hist, hist_diff), axis=1)
    combined_histogram = np.expand_dims(combined_histogram, axis=-1) # num_bins, 3, 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--subjects_list', nargs='+', help='List of items')
