...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample_to_eval(self, net, test_dataset, sample_path):
        ...
        raw_data_dir = self.config.data.dataset_config.raw_data_dir
        ...
        for pid in tqdm(batch_dict.keys()):
            out_path = os.path.join(sample_path, f'{pid}.nii.gz')
            """
            ...
            raw_image_path = raw_data_dir+'/'+str(pid)+self.config.data.dataset_config.CT_name

            # save sample as npy
            base_path_npy=str(self.config.result.ckpt_path)
            np.save(base_path_npy+str(pid)+".npy", sample)

            if 'BraTS' in sample_path:
                raw_image_path = os.path.join(raw_data_dir, pid, f"cropped_{pid}-t1n_preprocessed.nii")
            syn_img, raw_img, pid = save_syn_image(sample, raw_image_path, out_path, pid, self.config.data.dataset_config.plane)
            calcul_metrics(metrics_dict, pid, syn_img, raw_img)
        ...
        results_file = os.path.join('/data/pulkit/exvivo_image_translation/CT2MRI/results/ct2mr_160/test_results.csv')
