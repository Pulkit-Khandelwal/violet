...
def save_syn_image(images_3d, raw_image_path, out_path, pid, plane):
    raw_image = nib.load(raw_image_path)

    if plane == "coronal":
        image = torch.flip(images_3d, dims=[1])
        image = image.permute(2, 0, 1).numpy()
    elif plane == "axial":
        image = images_3d.permute(2, 1, 0)
    else:
        image = images_3d.permute(0, 2, 1)
        image = torch.flip(image, dims=[2])

    nii_img = nib.Nifti1Image(image, raw_image.affine, header=raw_image.header)
